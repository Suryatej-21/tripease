...
def createhotel():
    if session.get('hotel'):
        ...
        if request.method=="POST":
            ...
            path=os.path.dirname(os.path.abspath(__file__))
            static_path=os.path.join(path,'static')
            picture.save(os.path.join(static_path, filename))#hotel images
            images.save(os.path.join(static_path, filename1))#room images
...
def viewhotel_hotel():
    if session.get('hotel'):
        ...
        try:
            ...
            flash('No hotel or place details found.', 'error')
            return redirect(url_for('hlogin'))
    else:
        flash('Please log in to view hotel details.', 'error')
        return redirect(url_for('hlogin'))
...
def update_hotel(hotel_id):
    if request.method == 'POST':
        ...
        # Flash message to confirm the update
...
def delete_hotel(hotel_id):
    # Code to delete hotel with given hotel_id from the database
    execute_query('delete from bookings where hotel_id=%s',[hotel_id],commit=True)
    # Example: db.delete_hotel(hotel_id)
...
def viewbookings():
    if session.get('hotel'):
        hid_results = execute_query('SELECT hotel_id FROM hotels WHERE added_by = %s', [session['hotel']])
        if hid_results:
            hid = hid_results[0][0]
            details = execute_query('SELECT * FROM bookings WHERE hotel_id = %s', [hid])
            return render_template('viewbookingshotel.html', b=details)
        else:
            # Handle the case where no hotel ID is found for the session user
            flash('No hotel ID found for the current user', 'error')
            return redirect(url_for('hlogin'))
    else:
    ...
    if session.get('trip'):
        ...
        if request.method == "POST":
            ...
            execute_query('INSERT INTO trip_package (place_id, package_name, description, price, start_date, end_date, duration_days, includes_amenities, excludes_amenities, company_name, username) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',
                          [place_id, name, description, price, start_date, end_date, duration_days, includes_amenities, excludes_amenities, cname, session['trip']], commit=True)
